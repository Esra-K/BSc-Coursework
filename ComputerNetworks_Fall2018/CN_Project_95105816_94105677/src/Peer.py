...
"""
    ...
    This network is not completely decentralized but will show you some real-world challenges in Peer to Peer networks.
...
class Peer:
    def __init__(self, server_ip, server_port, is_root=False, root_address=None):
        ...
        Code design suggestions:
            1. initialize a Stream object for our Peer.
            2. initialize a PacketFactory object.
            3. initialize our UserInterface for interaction with user commandline.
            4. initialize a Thread for handling reunion daemon.
        ...
        self.daemon_thread = daemon_thread()  # start later with self.daemon_thread.start()
        if is_root:
            root = GraphNode((server_ip, server_port))
            self.net_graph = NetworkGraph(root)
    ...
    def run(self):
        ...
        Code design suggestions:
            1. Parse server_in_buf of the stream.
        ...
        Warnings:
            1. At first check reunion daemon condition; Maybe we have a problem in this time
               ...
               =========== time out for helloback from server ===========
    ...
    def run_reunion_daemon(self):
        ...
        Warnings:
            1. If we are the root of the network in the situation that we want to turn a node off, make sure that you will not
               advertise the nodes in sub-tree in our GraphNode ==============as parent node==============.
    ...
    def __handle_advertise_packet(self, packet):
        ...
        Code design suggestion: ============= Thread with a loop, every 4 secs. Run daemon? ============
    ...
    def __get_neighbour(self, sender):
        ...
        This function only will call when you are a root peer. ============ Graphnode =============
        ============ closest node to root with at most one child ===============
