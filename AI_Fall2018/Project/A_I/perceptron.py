...
mp = percptron(imagelength, data_size, len(labels))
...
for e in range(0, epochs):
    try:
        ...
        for i in range(0, iterations):
            ...
            forward = mp.train(X[batchstart:batchend], y[batchstart:batchend], batchsize, alpha)
            ...
            if i % outputinterval == 0 or i + 1 == iterations:
                print('\rEpoch %03d Loss: %.5f Alpha: %.5f ...  %.2f%%' %
                      (e,
...
# print(type(mp.W1))
# mp.W1 = np.load("W1.npy")
# mp.W2 = np.load("W2.npy")
# mp.W3 = np.load("W3.npy")
# mp.B1 = np.load("B1.npy")
# mp.B2 = np.load("B2.npy")
# mp.B3 = np.load("B3.npy")
