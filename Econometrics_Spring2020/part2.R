...
myvals <- c( "age", "big_team", "big_country", "release_clause_eur"
             , "attacker", "goalkeeping", "defending"
             , "potential"
             # , "weak_foot", "work_rate"
             # , "power", "movement"
...
fifa_selected2 <- fifa_selected2[, c(myvals, "value_eur")]
fifa_selected2 <- fifa_selected2[100:16880,]
...
cdat <- list(scale(fifa_selected2[, !(colnames(fifa_selected2) == 'joined')]))
names(cdat)[1] <- "x"
...
cc <- coeftest(lmMod2, vcov = vcovHC(lmMod2, "HC1"))
summary(cc)
